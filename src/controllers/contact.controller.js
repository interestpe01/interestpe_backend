...
exports.sendRequest = async (req, res) => {
  try {
    ...
    const request = await FriendRequest.create({ fromUserId: fromUser, toUserId: toUser.userId });
...
exports.respondRequest = async (req, res) => {
  try {
    ...
    if (status === "accepted") {
      await Contact.create({ userId: request.fromUserId, friendId: request.toUserId });
      await Contact.create({ userId: request.toUserId, friendId: request.fromUserId });
    }
...
exports.getFriends = async (req, res) => {
  try {
    const friends = await Contact.findAll({ where: { userId: req.user.userId } });
